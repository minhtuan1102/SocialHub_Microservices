...
const GroupMembersModal = ({ conversation, onClose, onGroupUpdated, friends }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-3 sm:p-4">
            <div className="bg-surface-container-lowest dark:bg-surface-container-high border border-outline-variant/20 rounded-2xl w-full max-w-md max-h-[90vh] flex flex-col overflow-hidden shadow-2xl animate-fadeIn">
                {/* Header */}
                <div className="flex items-center justify-between p-4 border-b border-outline-variant/10 bg-surface-container-low/60 dark:bg-surface-container-low/80">
                    <h3 className="font-bold text-on-surface text-sm">Quản lý thành viên nhóm</h3>
                    <button
                        onClick={onClose}
                ...
                <div className="p-4 sm:p-5 space-y-5 flex-1 overflow-y-auto">
                    ...
                    <div>
                        <h4 className="text-[10px] text-on-surface-variant font-bold uppercase tracking-wider mb-2">
                            Thành viên hiện tại ({currentMembers.length})
                        </h4>
                        <div className="max-h-40 overflow-y-auto space-y-2 pr-1">
                            {currentMembers.map((member) => (
                                <div key={member.userId} className="flex items-center justify-between p-2 hover:bg-surface-container-high/40 rounded-xl transition">
                                    <div className="flex items-center space-x-2.5">
                                        <img
                                            ...
                                            alt="Avatar"
                                        />
                                        <span className="text-xs font-semibold text-on-surface">{member.displayName}</span>
                                    </div>
                                    <div className="flex items-center space-x-2">
                                        <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${
                                            member.role === "admin" ? "bg-amber-500/10 text-amber-500 border border-amber-500/20" : "bg-surface-container-high text-on-surface-variant"
                                        }`}>
                                        ...
                                        {isCurrentUserAdmin && member.userId !== currentUser?.id && (
                                            <button
                                                ...
                                                className="p-1 hover:bg-error-container/30 text-on-surface-variant hover:text-error rounded-lg transition disabled:opacity-50 cursor-pointer"
                                            >
                        ...
                        {addableFriends.length > 0 ? (
                            <div className="max-h-44 overflow-y-auto space-y-2 pr-1">
                                {addableFriends.map((friend) => (
                                    <div key={friend.id} className="flex items-center justify-between p-2 hover:bg-surface-container-high/40 rounded-xl transition">
                                        <div className="flex items-center space-x-2.5">
                                            <img
                                                ...
                                                alt="Avatar"
                                            />
                                            <span className="text-xs font-semibold text-on-surface">{friend.displayName}</span>
                                        </div>
                                        <button
                                            disabled={isAdding}
                                            onClick={() => handleAddMember(friend.id)}
                                            className="flex items-center space-x-1 px-3 py-1.5 bg-primary hover:opacity-90 text-on-primary rounded-lg text-[10px] font-bold transition disabled:opacity-50 cursor-pointer shadow-sm"
                                        >
                            ...
                            <p className="text-[10px] text-on-surface-variant/60 italic text-center py-4">Tất cả bạn bè đã tham gia nhóm này.</p>
                        )}
                    </div>
                </div>
            </div>
